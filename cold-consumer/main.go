...
func main() {
	...
	cc, err := consumer.Consume(func(msg jetstream.Msg) {
		...
		fmt.Printf("[cold] id=%s lat=%.6f lon=%.6f geohash_12=%s geohash_9=%s geohash_7=%s geohash_5=%s timestamp=%d\n",
			event.ID, event.Lat, event.Lon, event.Geohash12, event.Geohash9, event.Geohash7, event.Geohash5, event.Timestamp)
